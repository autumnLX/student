    ...
    div.question-content
      ...
      div(class="option")
        div.question-option(v-for='(item, idx) in topic.topicOption' :key='item.option + new Date().getTime()')
          ...
          p.question-optionDesc(:class='handleOptionClass(item)' v-html='item.optionDesc.replace(/<img([^(src])*src="/g, "$&" + $config.resUrl)')
</template>

<script>
export default {
  name: 'QuestionTypeJsTdhxz',
