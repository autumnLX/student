<template lang="pug">
  div.question-type-js_lddw
    div.question-content(v-html='contentHtml')
</template>

<script>
export default {
  name: 'QuestionTypeJs_lddw',
  ...
  methods: {
    initRender() {
      this.contentHtml = this.topic.topicContent.replace(/<img([^(src])*src="/g, '$&' + this.$config.resUrl).replace(/\s*-\s*/g, ' ')
      this.currentDetail = {}
    },
    recordFn() {
      this.reftxt = this.topic.topicContent.replace(/<\/?[^>]*>/g, '').replace(/"/g, '\\"').replace(/\s*-\s*/g, ' ')
      this.initPRBtn('', 2)
