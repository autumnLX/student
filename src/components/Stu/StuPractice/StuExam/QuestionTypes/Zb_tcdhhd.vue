<template lang="pug">
  div.question-type-zb_tcdhhd
    div.question-content(v-for='(option, index) in topic.topicOption' :key='index + new Date().getTime()' :ref="index")
      div.question-stem
        span {{ totalIdx + index }}.
        p(v-html='handleTopicStem(option.topicStem)')
...
export default {
  ...
  methods: {
    handleTopicStem (topicStem) {
      return topicStem.replace(/\n/g, '<br>')
    },
    scroll (index) {
      this.$nextTick(() => {
  ...
  .question-content {
    ...
    .question-option {
      ...
      .question-checkBox {
        ...
        margin-right: 10px;
        cursor: pointer;
        background: url(/static/images/practice/exam/radio_normal.png) no-repeat
          center center;
      ...
      .question-optionItem.rightSelect,
      .question-optionDesc.rightSelect {
        color: #21bd73;
        * {
          color: #21bd73;
        }
      }
      .question-optionItem.wrongSelect,
      .question-optionDesc.wrongSelect {
        color: #ff6666;
        * {
          color: #ff6666;
        }
