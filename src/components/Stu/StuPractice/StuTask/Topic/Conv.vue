...
export default {
  ...
  methods: {
    ...
    scoreLeve(param) {
      ...
      this.$set(this.resultDetailList[this.currentIndex], 'recordAudioUrl', `http://${param.audioUrl.replace(/:8002/, '')}.mp3`)
      ...
      if (this.currentProcess === 2) { // 角色跟读
        ...
        if (this.work.practiceType === 'ld') {
          ...
          }, this.currentIndex, this.resultDetailList[this.currentIndex].recordAudioUrl)
        }
      }
    },
    scrollTOP() {
    ...
    render(details) {
      this.$set(this.randerContent[this.currentIndex], 'htmlContent', this.randerContent[this.currentIndex].content)
      this.$set(this.resultDetailList[this.currentIndex], 'label', this.randerContent[this.currentIndex].content)
      this.dataPersist({
