  ...
  div#BookPractice
    ...
    div.bookPractice(v-else-if='bookPracticeList.length')
      ...
      div.topic-practice(v-for='(o, idx) of bookPracticeList' :class='{expand: currentBookPraceticeId == o.id}')
        div.info-bar(@click='handleBookPracticeToggle(o.id, idx)' :class="{word: o.content_type == '1' && o.word.img, sent: o.content_type == '2' && o.word.img, para: o.content_type == '3' && o.word.img, dialog: o.content_type == '4' && o.word.img}")
          div.title-number.f18.grey6 {{idx + 1 + (currentPage -1) * 10}}.
          div.book-content(v-if="o.content_type == '4'")
            ul.text-content(v-if='o.word.randerConter.length')
              li(v-for='detail in o.word.randerConter')
                span.f18.grey6 {{detail.role}}:&nbsp;&nbsp;
                span.f18.grey6(v-html='detail.content')
            ul.text-content(v-else)
              li(v-for='detail in o.word.content')
                span.f18.grey6 {{detail.role}}:&nbsp;&nbsp;
                span.f18.grey6(v-html='detail.content')
          div.book-content(v-else)
            img(v-if='o.word.img' :src='o.word.img')
            p.text-content.f18.grey6(v-if="o.word.randerHtmlContent" v-html='o.word.randerHtmlContent')
            p.text-content.f18.grey6(v-else v-html='o.word.content')
        div.operation
          i.icon-common2.icon-trash(:class='{white: currentBookPraceticeId == o.id}' @click='deletComfirmZy(o.id)')
        div.pracetice-player(:class='{enter: currentBookPraceticeId == o.id, leave: !currentBookPraceticeId == o.id}')
          ...
          div.translateText(v-else)
            span.grey9.f14(v-if='o.word.randerWordSymbol' v-html='o.word.randerWordSymbol')
            span.grey9.f14(v-else-if='o.word.symbol') {{o.word.symbol}}  
            span.grey9.f14 {{o.word.translation}}
          div.video-operation
            i.play-icon(v-if="o.content_type == '4'" :class='{play: playerIconFlag === 0, playing: playerIconFlag === 1}' @click='playEvent(o)')
            i.play-icon(v-else :class='{disabled: !o.word.audio, play: playerIconFlag === 0 && o.word.audio, playing: playerIconFlag === 1 && o.word.audio}' @click='playEvent(o)')
            i.record-icon(:class='{record: recordIconFlag === 0, recording: recordIconFlag === 1}' @click='recordEvent(o, idx)')
              i.recording-bg(v-if='recordIconFlag === 1')
            i.replay-icon(v-if="o.content_type == '4'" :class='{replay: replayIconFlag === 0, replaying: replayIconFlag === 1}' @click='replayEvent(o, idx)')
            ...
            i.replay-icon(v-else :class='{replay: replayIconFlag === 0, replaying: replayIconFlag === 1}' @click='replayEvent(o, idx)')
    ...
    div.empty(v-else)
      i.emptyIcon
      span.f16.greyc {{$t('Stu.lowScore.emptyCollection')}}
  ...
  methods: {
    ...
    handleRecordResult (score, audioUrl, details) {
      if (this.type === 'conv') {
        ...
        let list = this.bookPracticeList[this.currentTopicIndex].word
        ...
        this.$set(this.bookPracticeList[this.currentTopicIndex].word, 'randerHtmlContent', randerHtmlContent)
      }
    },
    removeTag (tag) {
      if (tag) {
    ...
    label (details) {
      let text = ''
      let _new = ''
      let randerWordSymbol = ''
      if (this.type === 'conv') {
        text = this.bookPracticeList[this.currentTopicIndex].word.content[this.currentRecordIndex].content.replace(/-/g, ' ')
      } else {
        text = this.bookPracticeList[this.currentTopicIndex].word.content.replace(/-/g, ' ')
      }
      if (details[0].words) {
          ...
          if (this.type === 'word' && val.phone && val.phone.length > 0) {
            val.phone.forEach((phone, i) => {
              if (phone.score < this.$config.ScoreLevel.bad) {
                randerWordSymbol += `<span class='red'>${this.translation(phone.char)}</span>`
              } else if (phone.score < this.$config.ScoreLevel.good) {
                randerWordSymbol += `<span class='grey6'>${this.translation(phone.char)}</span>`
              } else {
                randerWordSymbol += `<span class='green'>${this.translation(phone.char)}</span>`
              }
            })
            if (randerWordSymbol.indexOf('[') < 0 && randerWordSymbol.indexOf(']') < 0) {
              randerWordSymbol = `[${randerWordSymbol}]`
            }
          }
          _new += _r;
        })
        this.$set(this.bookPracticeList[this.currentTopicIndex].word, 'randerWordSymbol', randerWordSymbol)
        return _new
      }
    },
    translation (char) {
    ...
    playEvent (param) {
      ...
      if (this.type === 'conv') { // 对话
        ...
        param.word.content.forEach((value, index) => {
          this.conversationAudioList[index] = {
            audioUrl: value.audio ? value.audio : value.stdAudioUrl
          }
        })
        url = this.conversationAudioList[0].audioUrl
      } else {
        url = param.word.audio
      }
      if (!url) {
        return
      }
    ...
    recordEvent (param, index) {
      ...
      this.initRecordStatus(() => {
        if (this.type === 'conv') {
          this.currentUrlIndex = 0
          this.doRecord(param.word.content[0].content, this.handleRecordResult)
        } else {
          this.doRecord(param.word.content, this.handleRecordResult)
        }
      })
    },
    ...
    getLowScoreZYList () {
      ...
      this.$ajax.post('/vocabs/getVocabList', {
        currentPage: this.currentPage
      }, (res) => {
        if (res.status === 1) {
          this.bookPracticeNonEmpty = false
          this.bookPracticeList = res.data.list.filter(o => { // 过滤脏数据
            return o.word !== 'null'
          })
          if (this.bookPracticeList.length) {
            this.bookPracticeList.forEach((value, i) => {
              let wordParam = JSON.parse(value.word)
              this.bookPracticeList[i].word = wordParam
              this.$set(wordParam, 'randerHtmlContent', ``)
              if (!this.bookPracticeList[i].word) {
                this.bookPracticeList[i].word = {
                  content: '',
                  symbol: '',
                  translation: ''
                }
              }
              if (wordParam.symbol && wordParam.symbol.indexOf('[') < 0 && wordParam.symbol.indexOf('[') < 0) {
                this.$set(wordParam, 'symbol', `[${wordParam.symbol}]`)
              }
              ...
              if (value.content_type === '4') {
                this.bookPracticeList[i].word.randerConter = []
                this.bookPracticeList[i].word.content.forEach((value, idx) => {
                  this.bookPracticeList[i].word.randerConter[idx] = {
        ...
        if (res.status === 1) {
          if (this.$refs.dialogParam.selectActiveFlag) {
            this.$ajax.post('/students/addCustomTagAlert', {
              ...
              if (res.status === 1) {
                this.checkCustomTagAlert('zy_collection')
              }
            })
          } else {
            this.checkCustomTagAlert('zy_collection')
      ...
      .pracetice-player {
        ...
        & > .video-operation {
          ...
          & > .play-icon {
            ...
            &.play {
              background-image: url("@{image-base-url}/stu/video/exercise_play.png");
            }

            &.play:hover {
              background-image: url("@{image-base-url}/stu/video/exercise_play_hover.png");
            }

            &.playing {
              background-image: url("@{image-base-url}/stu/video/exercise_playing.png");
              background-position: 0 0;
              animation: playing 1s steps(1) infinite 0s;
            }

            &.disabled {
              background-image: url("@{image-base-url}/stu/video/exercise_play_null.png");
              cursor: not-allowed;
            }
          }
          ...
          & > .replay-icon {
            ...
            &.replay {
              background-image: url("@{image-base-url}/stu/video/exercise_replay.png");
            }

            &.replay:hover {
              background-image: url("@{image-base-url}/stu/video/exercise_replay_hover.png");
            }

            &.replaying {
              background-image: url("@{image-base-url}/stu/video/exercise_replaying.png");
            }

            &.replaying:hover {
              background-image: url("@{image-base-url}/stu/video/exercise_replaying_hover.png");
            }
          }
        }
      }
