      ...
      tbody
        tr(v-for='(item, index) in rows' :key='"item"+ index')
          ...
          td.unitContent
            div
              div.left
                a.prac(@click='link(item)') {{item.competitionName}}
          ...
          td.operate
            div.operateArea
              div(
                ...
                @click='handleGotoPractice(item.stageInfo[0].canDo, item.competitionId, item.stageInfo[0].groupId, item.stageInfo[0].stageId, item.modelId, item.answerProcess, item.stageInfo[0].needSignUp)') 练习
              div(:class='{grey: item.stageInfo[0].isReport === 0, "green-line": item.stageInfo[0].isReport === 1}' @click='gotoReport(item.stageInfo[0].groupId, item.stageInfo[0].stageId, item.competitionId, item.stageInfo[0].reportType, item.stageInfo[0].paperId)') 报告
...
export default {
  ...
  methods: {
    ...
    link(param) {
      this.$router.push({
        ...
        query: {
          ...
          groupId: param.stageInfo[0].groupId,
          stageId: param.stageInfo[0].stageId
        }
      })
    },
    handleGotoPractice(canDo, cId, gId, sId, mId, answerProcess, needSignUp) {
      if (needSignUp) {
        this.$ajax.post('/postAssessment/post', { postAssessmentUrl: this.$config.jdcpIP + 'WebCompetition/setJoinTime', stage_id: sId }, (res) => {
          if (res.status === 1) {
            this.gotoPractice(canDo, cId, gId, sId, mId, answerProcess)
    ...
    gotoPractice(canDo, cId, gId, sId, mId, answerProcess) {
      if (canDo !== 0) {
        const _path = (this.$route.path + '?rIdx=2').replace(/\?/g, '{{q}}').replace(/&/g, '{{a}}')
        const stu = this.$store.state.student
        const _query = 'competitionId=' + cId +
          '&groupId=' + gId +
          '&stageId=' + sId +
          '&modelId=' + mId +
          '&user=' + stu.phone +
        ...
        if (parseInt(answerProcess) === 1) {
          window.location.href = `/web/models/ExamFormal/index.php?${_query}}`
        } else {
          window.location.href = `/practice/exam?${_query}`
        }
      }
    },
    gotoReport(gId, sId, cId, reportType, pId) { // 根据reportType字段判断报告逻辑
      if (reportType === 1) { // 免费报告
        this.goToBuyReport('', cId, gId, sId, 1, '', pId)
      } else if (reportType === 2) { // 分数报告
        this.goToBuyReport('', cId, gId, sId, 1, '', pId)
      } else if (reportType === 3) { // 诊断报告
    ...
    goToBuyReport(name, cId, gId, sId, free, city, pId) {
      ...
      if (typeof (window.ReportMapping.getReportMapping(sId)) === 'object') {
        this.$jsonp(window.ReportMapping.getController(sId, gId, studentId), {}, (err, res) => {
          if (err) {
            this.$store.commit('showErrDialog', {
